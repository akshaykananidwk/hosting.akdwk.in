...
$this->extend('layouts/admin'); $this->set('title', 'Clients');
$badge = ['active' => 'success', 'inactive' => 'muted', 'suspended' => 'warning', 'closed' => 'danger'];
?>

<div class="card">
    <div class="card-head">
        <span>Clients <span class="muted small">(Total <?= (int) ($meta['total'] ?? 0) ?>)</span></span>
        <a href="<?= e(url('admin/clients/create')) ?>" class="btn btn-primary btn-sm">+ New Client</a>
    </div>
    <div class="card-body">
        <form method="get" action="<?= e(url('admin/clients')) ?>" class="flex gap-1 mb-3">
            <input type="text" name="q" value="<?= e($q) ?>" placeholder="Search by email or name…">
            ...
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>Services</th><th>Credit</th><th>Status</th><th></th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (empty($clients)): ?>
                        <tr><td colspan="8" class="text-center muted">No clients found</td></tr>
                    <?php endif; ?>
                    <?php foreach ($clients as $c): ?>
                        <tr>
                            ...
                            <td class="text-right"><a class="btn btn-outline btn-sm" href="<?= e(url('admin/clients/' . $c['id'])) ?>">View</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <div class="flex gap-1 mt-3 items-center justify-between">
            ...
            <span class="flex gap-1">
                <?php if ($cur > 1): ?><a class="btn btn-outline btn-sm" href="<?= e(url('admin/clients') . '?page=' . ($cur - 1) . $ex) ?>">‹ Previous</a><?php endif; ?>
                <?php if ($cur < (int) $meta['last_page']): ?><a class="btn btn-outline btn-sm" href="<?= e(url('admin/clients') . '?page=' . ($cur + 1) . $ex) ?>">Next ›</a><?php endif; ?>
            </span>
        </div>
        <?php endif; ?>
    </div>
