...
$this->extend('layouts/admin'); $this->set('title', 'Transactions (વ્યવહાર)');
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="table-wrap">
            <table class="table">
                <thead><tr><th>#</th><th>ગ્રાહક</th><th>Invoice</th><th>Gateway</th><th>Amount</th><th>UTR</th><th>સ્થિતિ</th><th>Date</th><th></th></tr></thead>
                <tbody>
                    <?php if (empty($transactions)): ?><tr><td colspan="9" class="text-center muted">કોઈ transaction મળ્યો નહીં</td></tr><?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td class="text-right">
                                <?php if ($t['status'] === 'pending' && in_array($t['gateway'], $manual, true)): ?>
                                    <form method="post" action="<?= e(url('admin/transactions/' . $t['id'] . '/approve')) ?>" data-confirm="Approve કરવું છે?">
                                        <?= csrf_field() ?>
        ...
        <div class="flex gap-1 mt-3 items-center justify-between">
            ...
            <span class="flex gap-1">
                <?php if ($cur > 1): ?><a class="btn btn-outline btn-sm" href="<?= e(url('admin/transactions') . '?page=' . ($cur - 1) . $ex) ?>">‹ પાછળ</a><?php endif; ?>
                <?php if ($cur < (int) $meta['last_page']): ?><a class="btn btn-outline btn-sm" href="<?= e(url('admin/transactions') . '?page=' . ($cur + 1) . $ex) ?>">આગળ ›</a><?php endif; ?>
            </span>
        </div>
        <?php endif; ?>
    </div>
